          ...
          <div key={idx} className="flex items-center gap-2">
            <div className="flex flex-auto items-center gap-1 overflow-hidden">
              <input
                ...
                className="min-h-6 min-w-6 cursor-pointer rounded-full border! border-solid! border-setta-500/20! p-0 dark:border-setta-500/40! [&::-moz-color-swatch]:rounded-sm"
              />
              <input
                type="text"
                value={description}
                ...
                className="h-[calc(100%_-_2px)] w-[calc(100%_-_2px)] truncate rounded-sm border px-2 py-1 text-xs text-setta-700 focus:outline-hidden focus:ring-2 focus:ring-blue-500 dark:text-setta-400 dark:placeholder-setta-700"
              />
            </div>
            <button
  ...
  return colorChoices?.length > 0 ? (
    ...
    <>
      ...
      <input
        ...
        className="single-cell-child flex h-[calc(100%-2px)]! min-h-0 w-[calc(100%-2px)]!  min-w-0 cursor-pointer items-center justify-between overflow-hidden rounded-lg! border! border-solid! border-setta-500/20! dark:border-setta-500/40!"
        disabled={isDisabled}
        onKeyDown={(e) => {
          if (e.code === "Escape") {
            onEscape?.();
    ...
    <DropdownMenu.Root modal={false}>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          className="z-20 flex flex-col rounded-sm bg-white p-2 shadow-md dark:bg-setta-800"
          align="start"
        ...
        >
          {colorChoices.map((item, index) => (
            <DropdownMenu.Item
              key={index}
              className="flex cursor-pointer items-center rounded-sm px-2 py-1 hover:bg-setta-100 dark:hover:bg-setta-700"
